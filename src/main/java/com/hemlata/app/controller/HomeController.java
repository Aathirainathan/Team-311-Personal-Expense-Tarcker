...
public ModelAndView loginUser(ModelAndView modelAndView, User user) {
    ...
    String pass2 = existingUser != null ? existingUser.getPassword() : null;
    if (existingUser != null && pass1.equals(pass2)) {
        ...
        // wrong password or email does not exist
        modelAndView.addObject("message", "Incorrect email or password. Try again.");
        modelAndView.setViewName("login");
    }
    return modelAndView;
}
...
public ModelAndView loginSuccess(ModelAndView modelAndView) {
	 ...
	 * 
	 */
	@GetMapping({"/daily", "/monthly", "/yearly"})
	public String showReportCard(@ModelAttribute("expenseRecorder") ExpenseRecorder theExpenseRecorder,
	                             Model theModel,
	                             HttpServletRequest request) {
	    ...
	    switch (request.getRequestURI()) {
	        case "/daily":
	            ...
	            List<ICostAndDateQuery> costDateList = theExpenseService.getCostByDateList(
	                    theExpenseRecorder.getSelectedMonth(),
	                    theExpenseRecorder.getSelectedYear()
	            );
	            ...
	            YearMonth yearMonthObject = YearMonth.of(
	                    Integer.parseInt(theExpenseRecorder.getSelectedYear()),
